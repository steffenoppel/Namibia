...
dat2 <- read_excel("Namibia demersal trawl Data_October 2018.xlsx", 
                  sheet="6_Interactions")[,1:19]
...
dat$h.lub <- hour(dat$t.lub) + minute(dat$t.lub)/60
...
# define seasons
dat$Month <- months(dat$Date_Start_Observation)
dat$Season <- "Summer"
...
rawsummary<-NAM.trawl %>% mutate(count=1) %>% group_by(Tori_line_deployed,Vessel_Activity) %>%
...
##############################################################
#### PRODUCE BOOTSTRAPPED CONFIDENCE INTERVALS 
##############################################################
