...
## BRANCHED THIS CODE FROM FULL ANALYSIS DUE TO CONCERNS OVER FOA DATA
## Nina found on 18 May 2020 that FOA observers report random bullshit in terms of effort - not reliable
## we test whether we find same reduction if only using ATF data
...
NamLLmodel <- jagsUI(data=jags.data,
                  model = "C:/STEFFEN/RSPB/Marine/Bycatch/Namibia/Data/ATF_Nam_LongLine_Bycatch_v5.jags",
                  inits=inits,
...
fwrite(LLsum,"Namibia_longline_bycatch_REG_comparison_ATF_only.csv")
